...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        if (!id.match(/^[0-9a-fA-F]{24}$/)) {
            throw (0, errorHandler_1.createError)('Invalid invoice ID format', 400);
        }
        const invoice = await Invoice_1.default.findById(id);
        if (!invoice) {
            throw (0, errorHandler_1.createError)('Invoice not found', 404);
        }
        res.json({
            success: true,
            data: invoice,
...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (!fileId || !fileName || !vendor || !invoice) {
            throw (0, errorHandler_1.createError)('Missing required fields: fileId, fileName, vendor, invoice', 400);
        }
        // Create new invoice
...
router.put('/:id', async (req, res, next) => {
    try {
        ...
        const updatedInvoice = await Invoice_1.default.findByIdAndUpdate(id, req.body, { new: true, runValidators: true });
        if (!updatedInvoice) {
            throw (0, errorHandler_1.createError)('Invoice not found', 404);
        }
        res.json({
...
router.delete('/:id', async (req, res, next) => {
    try {
        ...
        const deletedInvoice = await Invoice_1.default.findByIdAndDelete(id);
        if (!deletedInvoice) {
            throw (0, errorHandler_1.createError)('Invoice not found', 404);
        }
        res.json({
