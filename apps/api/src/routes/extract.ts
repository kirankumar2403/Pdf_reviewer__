...
// Returns an empty string on failure instead of throwing, so callers can gracefully fallback
async function extractTextFromPDF(filePath: string): Promise<string> {
  try {
    ...
    // Known pdf.js error patterns like "bad XRef entry" should not crash the flow
    return ''
  }
}

// Helper function to process text with Gemini AI
async function processWithGemini(text: string, apiKey: string) {
  try {
    ...
    if (provider === 'gemini') {
      ...
      try {
        ...
        if (fileExists) {
          ...
          if (!text) {
            // Graceful fallback when pdf-parse fails (e.g., bad XRef entry)
            const errorMessage = 'Failed to read PDF text (possibly corrupted or unsupported format)'
            console.warn(`⚠️  ${errorMessage}:`, fileId)
            extractedData = {
              _id: undefined,
              fileId: fileId,
        ...
        // Create a fallback extraction result instead of throwing a 500 error
        if (!extractedData) {
          ...
          extractedData = {
            ...
            extractionError: `Error processing PDF: ${errorMsg}`,
            createdAt: new Date().toISOString(),
          }
        }
      }
      ...
      throw createError('Invalid provider specified', 400)
    }
